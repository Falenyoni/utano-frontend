...
export function InvoiceDetailModal({ invoiceId, onClose }: Props) {
  ...
  const createPaymentPlan = useCreatePaymentPlan(invoiceId)
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 z-40 flex items-start justify-center p-4 overflow-y-auto"
      onClick={(e) => { if (e.target === e.currentTarget) onClose() }}
    >
      <div className="relative bg-white dark:bg-gray-950 rounded-xl shadow-2xl w-full max-w-4xl my-4 border border-gray-200 dark:border-gray-800">
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 h-8 w-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-200 transition-colors text-lg leading-none"
          aria-label="Close"
        >
          ✕
        </button>
        ...
        {!isLoading && !invoice && (
          <div className="p-10 text-center text-red-500 dark:text-red-400">Invoice not found.</div>
        )}
                  ...
                  <div className="px-4 py-3 space-y-2 text-sm">
                    ...
                    {invoice.medAidClaimStatus === 'None' && (
                      <p className="text-xs text-gray-400 dark:text-gray-500 pt-1">
                        ...
                        <Link to="/claims" onClick={onClose} className="text-blue-600 dark:text-blue-400 hover:underline">Claims</Link>{' '}
                        to submit.
                      </p>
                    )}
                  </div>
